...
> 此目錄保存正式產品開發前完成的 React 前端展示版，並可獨立安裝、執行、測試與建置。
...
在 `demo/` 目錄雙擊 `start-gym-assistant.cmd`。它會在需要時安裝依賴、啟動本機伺服器，並自動開啟 `http://127.0.0.1:5173/today`。使用期間請保留命令視窗開啟；關閉視窗即會停止本機伺服器。
